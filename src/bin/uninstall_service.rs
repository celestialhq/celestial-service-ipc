...
fn main() -> Result<(), Error> {
    ...
    let service_id = celestial_service_ipc::MACOS_SERVICE_ID;
...
fn main() -> Result<(), Error> {
    ...
    let _gate = enter_repair_gate()?;
    let debug = env::args().any(|arg| arg == "--debug");
    let service_name = celestial_service_ipc::SERVICE_SLUG;
    ...
    let target =
        celestial_service_ipc::prepare_service_install_directory()?.join("celestial-service");
    if target.exists() {
